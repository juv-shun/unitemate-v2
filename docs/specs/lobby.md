...
## 着席タイムアウト
- マッチ成立（`match.created_at`）から **5分** 経過しても全員着席していない場合、通知モーダルを自動表示
- 自分が着席済みの場合のみ、カウントダウン（残り時間）をヘッダー付近に表示
- 自分が未着席の場合はカウントダウン非表示（着席前モーダルが表示されたまま）
- 全員着席するとカウントダウンは非表示になり、タイムアウト処理は発生しない
- タイムアウト通知モーダル:
  - 「5分経過しました。未着席ユーザーがいるので、無効試合として退出してください。必要であれば、未着席ユーザーを通報してください。」のメッセージを表示
  - 退出処理は行わない（通知のみ）
  - 「閉じる」ボタンでモーダルを閉じる
  - 一度閉じたら再表示されない
  - 退出する場合は、通常の「試合終了 / 退出」ボタンを使用
